...
public class Client {
	   ...
	   /**
	    * sends data and waits to receive an echo reply
	    */
	   private void sendAndReceive(byte[] msg) {
    	  ...
    	  System.out.println("Client Sending to host: ");
	      ...
	      System.out.println("Client trying to receive echo from host \n");
	      try {
	   ...
	   /**
	    * sends request for data and waits to receive it 
	    */
	   private void requestUpdate() {
	    	  byte[] msg = new String("client requesting acknowledgement").getBytes();
	    	  try {
	 	         sendPacket = new DatagramPacket(msg, msg.length,
	 	                                         InetAddress.getLocalHost(), 23);
	 	      } catch (UnknownHostException e) {
	    	  ...
	    	  System.out.println("Requesting update from host: ");
	    	  Utils.printPacketInfo(sendPacket);
		      try {
		    	  sendReceiveSocket.send(sendPacket);
			  } catch (IOException e) {
		      ...
		      System.out.println("client trying to receive acknowledgement \n");
		      try {
		         // Block until a datagram is received via sendReceiveSocket.  
		         sendReceiveSocket.receive(receivePacket);
		         System.out.println("received");
